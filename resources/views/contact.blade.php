            ...
            <form method="POST" action="{{ route('contact.submit') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="message" name="message" rows="5"
                              ...
                              class="w-full px-3.5 py-3 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-indigo-400 transition resize-y">{{ old('message') }}</textarea>
                    @error('message')
                        <span class="block text-red-500 text-xs mt-1.5">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <button type="submit"
                        class="w-full py-3.5 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 transition">
                    Send Message
                </button>
            </form>
        </div>
